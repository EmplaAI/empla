...
export function EmployeeEditDialog({ employee, open, onOpenChange }: EmployeeEditDialogProps) {
  ...
  const onSubmit = async (data: EditFormData) => {
    ...
    // objects generated by the LLM, not preset strings. Preservation
    // rules, in precedence order:
    //   1. Admin picked an explicit preset in the dropdown (not 'default')
    //      → honor the choice and send `{ preset: ... }`, overriding
    //      the slider shape. The UI leaves the selector interactive, so
    //      silently ignoring the pick would be a lie.
    //   2. Existing personality is slider-shaped AND admin left the
    //      dropdown at 'default' → preserve the sliders unchanged so
    //      an innocent name/email edit doesn't silently reset the LLM-
    //      generated values.
    //   3. Otherwise → fall back to preset-or-empty.
    const existingPersonality = (employee.personality ?? {}) as Record<string, unknown>;
    const isCustomPersonality =
      'openness' in existingPersonality && !('preset' in existingPersonality);
    const hasExplicitPreset =
      !!data.personalityPreset && data.personalityPreset !== 'default';
    const personality: Record<string, unknown> = hasExplicitPreset

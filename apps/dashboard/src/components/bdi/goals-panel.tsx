...
import { useGoals, type EmployeeGoal } from '@empla/react';
...
function GoalItem({ goal }: { goal: EmployeeGoal }) {
  return (
    <div className="flex items-start gap-3 rounded-lg border border-border/50 bg-muted/30 p-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
          <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${STATUS_STYLES[goal.status] ?? STATUS_STYLES.active}`}>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-8 text-center">
      ...
      <p className="text-xs text-muted-foreground">Goals will appear as the employee operates</p>
    </div>
  );
}
...
export function GoalsPanel({ employeeId }: { employeeId: string }) {
  ...
  const { data, isLoading, error, refetch } = useGoals(employeeId, {
    page,
    pageSize: PAGE_SIZE,
    autoRefresh: true,
    interval: 30,
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Failed to load goals</p>
